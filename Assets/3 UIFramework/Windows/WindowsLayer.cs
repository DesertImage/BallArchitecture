...
namespace DesertImage.UI
{
    public class WindowsLayer : Layer<ushort, IWindow>
    {
        ...
        private void ShowProcess(WindowEntry entry, bool animate = true)
        {
            ...
            else if (Current != null && !Current.DontHideIfNotForeground && !entry.Window.IsPopup)
            {
                Current.Hide();
            }
        ...
        public override void HIde(IWindow screen, bool animate = true)
        {
            ...
            else if (lastScreen?.IsPopup ?? false)
            {
                if (History.Count > 0)
                {
        ...
        private void OnScreenEnterAnimationFinished(IScreen<ushort> screen)
        {
            RemoveTransition((IWindow)screen);
        }

        private void OnScreenExitAnimationFinished(IScreen<ushort> screen)
        {
            RemoveTransition((IWindow)screen);
