...
namespace DesertImage.Extensions
{
    public static class EnumExtensions
    {
        ...
        public static T GetRandomValue<T>(this object sender, ushort minIndex, ushort maxIndex, T exceptItem = default) where T : Enum
        {
            var values = Enum.GetValues(typeof(T));
        ...
        public static T GetValueByName<T>(this object sender, string name = "") where T : Enum
        {
            ...
            return (from objId in ids where objId.ToString().ToLower().Contains(name.ToLower()) select objId).FirstOrDefault();
        }
    }
}